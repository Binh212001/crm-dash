...
import { IsUsername } from "@/decorators/validators/is-usename.decorator";
...
export function UserNameField(
  options: Omit<ApiPropertyOptions, "type" | "minLength" | "maxLength"> &
    IStringFieldOptions = {}
): PropertyDecorator {
  const decorators = [
    StringField({ ...options, minLength: 5, maxLength: 50 }),
    IsUsername(),
  ];
...
export function UUIDField(
  ...
  const decorators = [Type(() => String), IsUUID("7", { each: options.each })];
...
export function SlugField(
  ...
  const decorators = [
    Type(() => String),
    Matches(slugRegex, { each: options.each }),
  ];
...
export function SlugFieldOptional(
  options: Omit<ApiPropertyOptions, "type" | "required" | "isArray"> &
    IFieldOptions = {}
): PropertyDecorator {
  return applyDecorators(
    IsOptional({ each: options.each }),
    SlugField({ required: false, ...options })
  );
}
